...
public abstract class NetherNetChannel extends AbstractChannel {
    ...
    private final AtomicBoolean loggedFirstInboundPayload = new AtomicBoolean(false);
    private final AtomicBoolean loggedFirstOutboundPayload = new AtomicBoolean(false);
                ...
                if (segments == 0) {
                    try {
                        if (assemblyBuf.isReadable()) {
                            ...
                            if (loggedFirstInboundPayload.compareAndSet(false, true)) {
                                log.info("First inbound payload on {} bytes={}",
    ...
    private void writeInternal(Object msg) {
        ...
        if (loggedFirstOutboundPayload.compareAndSet(false, true)) {
            log.info("First outbound payload on {} bytes={}",
                    getClass().getSimpleName(),
